...
class Prompts(BaseSettings):
    ...
    router_prompt_v2: str = (
        ...
        **3. Перевод или объяснение одного корейского слова**
        - Пользователь явно запрашивает перевод или объяснение одного корейского слова.
        - **Решение:**
            - Определите точное слово, нуждающееся в объяснении или переводе.
            - Выведите только слово в исходной форме (без дополнительных комментариев).
        - Установите: `mode = "vocab"`
        
        **4. Запрос на перевод текста**
        ...
        **Описание:**  
        {description (выделяйте корейскую грамматику жирным шрифтом)}
        
        **Форма:**  
        {usage_form}
        
        **Примеры:**
        {examples[a] korean (выделяйте грамматическую конструкцию)}  
        *{examples[a] russian}*
